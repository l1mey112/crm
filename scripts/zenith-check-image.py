...
def check(image):
    ...
    if "manifests" in manifest:
        candidates = [item for item in manifest["manifests"] if item.get("platform", {}).get("os") == "linux" and item.get("platform", {}).get("architecture") == "amd64"]
        if not candidates:
    ...
    return {"image": f"{registry}/{name}@{top_digest}", "digest": top_digest, "platform": "linux/amd64", "manifest_digest": selected_digest, "config_digest": config_digest}


if __name__ == "__main__":
    try:
        if len(sys.argv) != 2:
            raise CheckError("Usage: zenith-check-image.py registry/repository:tag-or-@digest")
        print(json.dumps(check(sys.argv[1]), indent=2))
    except urllib.error.HTTPError as error:
        sys.exit(f"Registry check failed: HTTP {error.code}")
    except (CheckError, ValueError, KeyError, TypeError, AttributeError, OSError) as error:
